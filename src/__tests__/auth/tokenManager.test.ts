...
describe('TokenManager', () => {
  ...
  describe('token management', () => {
    it('should set and get token for a company', async () => {
      ...
      expect(token?.expires_at).toBe(mockTokenResponse.created_at + mockTokenResponse.expires_in);
    });
    ...
    it('should get all company IDs', async () => {
      await tokenManager.setToken(123, mockTokenResponse);
      await tokenManager.setToken(456, { ...mockTokenResponse, access_token: 'test_access_token_2' });
  ...
  describe('token expiration', () => {
    ...
    it('should get token expiry status', () => {
      ...
      expect(tokenManager.getTokenExpiryStatus(expiredToken).status).toBe('expired');
      expect(tokenManager.getTokenExpiryStatus(nearExpiryToken).status).toBe('near_expiry');
      expect(tokenManager.getTokenExpiryStatus(validToken).status).toBe('valid');
    });
  });
});