  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          'Get cash flow statement - NOT AVAILABLE: freee API does not provide a cash flow endpoint. Calculate cash flow from P&L and balance sheet changes instead.',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    case 'freee_get_partners': {
      ...
      const partners = await freeeClient.getPartners(
        getCompanyId(params.companyId),
        {
          name: params.name,
    ...
    case 'freee_create_partner': {
      ...
      const partner = await freeeClient.createPartner(
        getCompanyId(params.companyId),
        {
          name: params.name,
          shortcut1: params.shortcut1,
    ...
    case 'freee_get_sections': {
      ...
      const sections = await freeeClient.getSections(
        getCompanyId(params.companyId),
      );
      return {
        content: [
    ...
    case 'freee_get_invoices': {
      ...
      const invoices = await freeeClient.getInvoices(
        getCompanyId(params.companyId),
        {
          partner_id: params.partnerId,
    ...
    case 'freee_create_invoice': {
      ...
      const invoice = await freeeClient.createInvoice(
        getCompanyId(params.companyId),
        {
          issue_date: params.issueDate,
          partner_id: params.partnerId,
