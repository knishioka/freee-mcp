...
export class FreeeClient {
  ...
  async checkTaggingConsistency(
    ...
    // --- Partner-level tag consistency (counted per deal, not per detail) ---
    interface DealTagInfo {
      hasTag: boolean;
      tagKey: string | null;
    }
    const partnerDeals = new Map<
      number,
      { name: string; dealTags: DealTagInfo[] }
    >();
    for (const deal of deals) {
      ...
      // Collect all unique tag IDs across the deal's details
      const allTagIds = new Set<number>();
      for (const d of deal.details) {
        if (d.tag_ids) {
          for (const id of d.tag_ids) allTagIds.add(id);
        }
      }
    ...
    for (const [partnerId, { name, dealTags }] of partnerDeals) {
      const taggedCount = dealTags.filter((d) => d.hasTag).length;
      const untaggedCount = dealTags.length - taggedCount;

      // Count tag pattern frequencies (per deal)
      const patternCounts = new Map<string, number>();
      for (const d of dealTags) {
        if (d.tagKey) {
          patternCounts.set(d.tagKey, (patternCounts.get(d.tagKey) ?? 0) + 1);
        }
      }

      const hasInconsistency = untaggedCount > 0 || patternCounts.size > 1;
      ...
      if (hasInconsistency) {
        ...
        tagInconsistencies.push({
          ...
          partner_name: name,
          total_deals: dealTags.length,
          tagged_deals: taggedCount,
          untagged_deals: untaggedCount,
